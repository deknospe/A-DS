...
template <typename T, T(*func)(T, T), T neutralElem> class ArrayBasedSegmentTree {
private:
    ...
    struct Node {
        ...
        explicit Node(const T &value, const size_t l, const size_t r)
            : value(value)
            , lProjection(l)
            , rProjection(r)
        {}
...
public:
        ...
        , arr(std::vector<Node*>(2 * this->size))
    {
        for (size_t i = 0; i < arr.size(); ++i) {
            this->arr[i + this->size] = new Node(arr[i], i + this->size, i + 1 + this->size);
        }
        for (size_t i = arr.size(); i < this->size; ++i) {
            this->arr[i + this->size] = new Node(neutralElem, i + this->size, i + 1 + this->size);
        }
    ...
    explicit ArrayBasedSegmentTree(const size_t size) noexcept
